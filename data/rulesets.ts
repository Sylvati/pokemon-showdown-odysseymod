...
export const Rulesets: import("../sim/dex-formats").FormatDataTable = {
	...
	odysseyag: {
		name: "Odyssey AG",
		desc: "The minimal ruleset for Anythign Goes, with only Odyssey mons.",
		ruleset: [
			"+ODY AG",
			"Team Preview",
			"HP Percentage Mod",
	...
	batonpasstrapclause: {
		...
		onBegin() {
			this.add(
				"rule",
				"Baton Pass Trap Clause: No Baton Passer may have a way to trap Pok\u00e9mon"
			);
		},
		onValidateTeam(team, format, teamHas) {
			...
			const bpAndTrap = team.some((set) => {
				...
				return (
					set.moves.includes("batonpass") &&
					set.moves.some((move) => trappingMoves.includes(move))
				);
			});
			if (bpAndTrap) {
				return [
			...
			if (gen >= 3) {
				...
				if (species.abilities["H"] && species.abilities["S"]) {
					buf += `<span class="col twoabilitycol${
						species.unreleasedHidden ? " unreleasedhacol" : ""
					}"><em>${species.abilities["H"]}<br />(${
						species.abilities["S"]
					})</em></span>`;
				} else if (species.abilities["H"]) {
					buf += `<span class="col abilitycol${
						species.unreleasedHidden ? " unreleasedhacol" : ""
					}"><em>${species.abilities["H"]}</em></span>`;
